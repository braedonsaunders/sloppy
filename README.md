...
Add this to `.github/workflows/sloppy.yml`. Get a code quality score on every PR. Free.
...
Want it to actually *fix* the issues? Bring your own API key and add fix mode. Keep reading.
...
Sloppy has two modes: **scan** (free, report only) and **fix** (BYOK, auto-fix + PR). They are designed to run separately — scan on every PR, fix on-demand or on a schedule.

> **Important:** Do not run scan and fix in the same workflow at the same time. Scan is fast and free — use it as a CI check. Fix is slow and costs money — run it manually or on a schedule.
...
What you get:
- Score from 0 to 100 on every PR
...
Create `.github/workflows/sloppy.yml`:
...
This should be a **separate workflow file** from your scan. It runs on-demand (`workflow_dispatch`) or on a schedule — not on every push or PR.

Create `.github/workflows/sloppy-fix.yml`:
...
**With OpenAI Codex:** same workflow, just swap the agent and key:
...
Store it as a GitHub secret named `CLAUDE_CODE_OAUTH_TOKEN`, then use the same fix workflow as Tier 2 but with the OAuth token instead of an API key:
...
**Important:** Tokens generated by `claude setup-token` are long-lived (~1 year expiry). They do not need frequent regeneration. Max subscription usage is subject to your plan's rate limits (5-hour rolling window + weekly ceiling). For high-volume or team usage, API keys are more reliable.

### Both in one repo (recommended setup)

Most users will want both: free scan on PRs, fix on-demand. Create two separate workflow files:

```
.github/
  workflows/
    sloppy.yml       # Tier 1: scan on every PR (free)
    sloppy-fix.yml   # Tier 2/3: fix on-demand or weekly (BYOK)
```

This keeps them completely independent. The scan never triggers a fix run, and the fix never blocks your PRs.
...
Scan mode analyzes your code and reports a score. Fix mode uses an AI coding agent (Claude Code or OpenAI Codex) to find and fix issues, then opens a pull request with atomic commits. Scan is free and fast — run it on every PR. Fix costs money and takes time — run it manually or on a weekly schedule. They should be **separate workflow files**, not two jobs in the same workflow.

**Q: Should I run scan and fix at the same time?**

No. They serve different purposes and should trigger on different events. Put scan in `sloppy.yml` triggered by `pull_request`. Put fix in `sloppy-fix.yml` triggered by `workflow_dispatch` and/or `schedule`. See the [How It Works](#how-it-works) section for the recommended setup.
